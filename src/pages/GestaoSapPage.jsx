import { Box, Card, CardContent, Typography, Grid, TextField, MenuItem, Button, Chip } from '@mui/material';
import { Search } from '@mui/icons-material';
import { useState } from 'react';
import { empresas } from '../data/mockData';

const tiposCodigo = ['Material / Telecom', 'Serviço'];
const distribuidoras = ['Neoenergia Coelba', 'Neoenergia Pernambuco', 'Neoenergia Cosern', 'Neoenergia Brasília', 'Neoenergia Elektro'];
...
export default function GestaoSapPage() {
  ...
  return (
    <Box>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h4" fontWeight={700} color="primary.dark">Gestão SAP</Typography>
        <Typography variant="body2" color="text.secondary">Consulte e gerencie códigos SAP de materiais e serviços</Typography>
      </Box>

      <Card sx={{ mb: 3 }}>
        <CardContent sx={{ p: 3 }}>
          <Typography variant="h6" fontWeight={700} color="primary.dark" sx={{ mb: 2 }}>Buscar Código SAP</Typography>
          <Grid container spacing={2.5} alignItems="flex-end">
            <Grid item xs={12} sm={4}>
              <TextField label="Tipo" select fullWidth value={tipo} onChange={(e) => setTipo(e.target.value)}>
                <MenuItem value="">Todos</MenuItem>
                {tiposCodigo.map((t) => <MenuItem key={t} value={t}>{t}</MenuItem>)}
              </TextField>
            </Grid>
            <Grid item xs={12} sm={4}>
              <TextField label="Distribuidora" select fullWidth value={empresa} onChange={(e) => setEmpresa(e.target.value)}>
                <MenuItem value="">Todas</MenuItem>
                {distribuidoras.map((d) => <MenuItem key={d} value={d}>{d}</MenuItem>)}
              </TextField>
            </Grid>
            <Grid item xs={12} sm={4}>
              <TextField
          ...
          <Box sx={{ mt: 3 }}>
            <Button variant="contained" startIcon={<Search />} onClick={handleBuscar}>Buscar</Button>
          </Box>
        </CardContent>
      </Card>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              {resultados.length === 0 ? (
                ...
                resultados.map((r) => (
                  ...
                  >
                    <Typography variant="body1" fontWeight={700} color="primary.dark" sx={{ minWidth: 90 }}>{r.codigo}</Typography>
                    <Typography variant="body2" sx={{ flex: 1 }}>{r.descricao}</Typography>
